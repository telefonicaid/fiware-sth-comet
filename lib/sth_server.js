...
function startServer(host, port, aSTHDatabase, callback) {
   ...
   */
  function storeRawData(data, reply) {
  ...
  /**
   * Creates the handler called once the notification info has been generated
   * @param data Object including the following properties:
   *  - contextElement The context element associated to the notification
   *  - attribute The attribute associated to the notification
   *  - recvTime The reception time associated to the notification
   *  - counterObj The counter object
   *  - totalTasks The total number of tasks to execute
   * @param reply hapi's server reply() function
   * @param err Error in case of error
   * @param result The notification info in case no error occured
   * @return The created handler
   */
  function createOnNotificationInfo(data, reply) {
    return function onNotificationInfo(err, result) {
      if (!err && !result.exists) {
        // Include the received notification info into the data
        data.notificationInfo = result;

        // Store the raw data into the database
        storeRawData(data, reply);
        ...
        if (result.exists) {
          sthLogger.debug(
            ...
            'Ignoring then notification since already registered'
          );
        }
