      ...
      function(err) {
        if (err) {
          if (err.name === 'MongoError' && err.message.indexOf('does not exist. Currently in strict mode')) {
            ...
            sthLogger.debug(
              request.sth.context,
              'No data associated to the provided service (' + request.headers['fiware-service'] +
                '), service path (' + request.headers['fiware-servicepath'] + ')' +
                ', entity id (' + request.params.entityId + ')' +
                ', entity type (' + request.params.entityType + ')' +
        ...
        sthLogger.debug(
          request.sth.context,
          'Data associated to the provided service (' + request.headers['fiware-service'] +
            '), service path (' + request.headers['fiware-servicepath'] + ')' +
