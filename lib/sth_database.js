...
/**
 * Creates the handler to manage the result of aggregated data removal
 * @param callback The callback to notify
 * @param err Error when processing the aggregated data removal, if any
 * @return {Function} The generated handler
 */
function createOnAggregatedDataRemoved(callback) {
  return function onAggregatedDataRemoved(err) {
    if (callback) {
...
/**
 * Creates the handler to manage the result of aggregated data storage
 ...
 * @return {Function} The generated handler
 */
function createOnAggregatedDataStored(data, callback) {
  return function onAggregatedDataStored(err) {
    ...
    if (data.notificationInfo.updates) {
      data.collection.update(
        ...
        createOnAggregatedDataRemoved(callback)
      );
    } else {
...
/**
 * Creates the handler to manage the result of the prepopulation of the aggregated data
 * @param {object} data The data to be stored. It is an object including the following properties:
 *  - {object} collection: The collection where the data should be stored
 *  - {string} entityId The entity id
 *  - {string} entityType: The entity type
 ...
 * @return {Function} The generated handler
 */
function createOnAggregatedDataPrepopulated(data, callback) {
  return function onAggregatedDataPrepopulated(err) {
    if (err && callback) {
      return process.nextTick(callback.bind(null, err));
    }
    ...
    data.collection.update(
      ...
      createOnAggregatedDataStored(data, callback)
    );
  };
}
...
function storeAggregatedData4Resolution(data, callback) {
  ...
  */
  ...
  collection.update(
    ...
    createOnAggregatedDataPrepopulated(data, callback)
  );
}
...
/**
 * Creates the handler which analyzes the search of already existent raw data and invokes the passed callback with the
 * results (this is, the information about the notification)
 * @param {object} data The data to be stored. It is an object including the following properties:
 *  - {object} collection: The collection where the data should be stored in
 *  - {date} recvTime The date the event arrived
 *  - {string} entityId The entity id associated to the updated attribute
 ...
 * @return {Function} The generated handler
 */
function createOnAlreadyRegisteredDataSearch(data, callback) {
  return function onAlreadyRegisteredDataSearch(err, result) {
    if (err && callback) {
      return process.nextTick(callback.bind(null, err));
    }
    if (result) {
      if (result.attrValue !== data.attribute.value) {
        // The notification is an already existent data update
        if (sthHelper.getAggregationType(result.attrValue) === sthConfig.AGGREGATION.TYPES.NUMERIC) {
          getNewMaxValues(data, function (err, newMaxValues) {
            if (err && callback) {
              return process.nextTick(callback.bind(null, err, {updates: result}));
            }
            if (newMaxValues) {
              getNewMinValues(data, function (err, newMinValues) {
                ...
                return process.nextTick(
                  callback.bind(
                    null, err, {updates: result, newMinValues: newMinValues, newMaxValues: newMaxValues}));
              });
            } else {
              return process.nextTick(callback.bind(null, err, {updates: result}));
            }
        ...
        return process.nextTick(callback.bind(null, err, {exists: result}));
      }
    } else {
      // The notification is an new data insertion
      return process.nextTick(callback.bind(null, err, {inserts: true}));
    }
  };
}
...
function getNotificationInfo(data, callback) {
  ...
  collection.findOne(
    findCondition,
    createOnAlreadyRegisteredDataSearch(data, callback)
  );
}
