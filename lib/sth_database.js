...
/**
 * Handler to manage the result of aggregated data removal
 * @param callback The callback to notify
 * @param err Error when processing the aggregated data removal, if any
 */
function onAggregatedDataRemoved(callback, err) {
  if (callback) {
...
/**
 * Handler to manage the result of aggregated data storage
 ...
 */
function onAggregatedDataStored(data, callback, err) {
  ...
  if (data.notificationInfo.updates) {
    data.collection.update(
      ...
      onAggregatedDataRemoved.bind(null, callback)
    );
  } else {
...
/**
 * Handler to manage the result of the prepopulation of the aggregated data
 * @param {object} data The data to be stored. It is an object including the following properties:
 *  - {object} collection: The collection where the data should be stored
 *  - {string} entityId The entity id
 *  - {string} entityType: The entity type
 ...
 * @param {Function} callback Function to call once the operation completes
 */
function onAggregatedDataPrepopulated(data, callback, err) {
  if (err && callback) {
    return process.nextTick(callback.bind(null, err));
  }
  ...
  data.collection.update(
    ...
    onAggregatedDataStored.bind(null, data, callback)
  );
}
...
function storeAggregatedData4Resolution(data, callback) {
  ...
  */
  ...
  collection.update(
    ...
    onAggregatedDataPrepopulated.bind(null, data, callback)
  );
}
...
/**
 * Analyzes the search of already existent raw data and invokes the passed callback with the results
 *  (this is, the information about the notification)
 * @param {object} data The data to be stored. It is an object including the following properties:
 *  - {object} collection: The collection where the data should be stored in
 *  - {date} recvTime The date the event arrived
 *  - {string} entityId The entity id associated to the updated attribute
 ...
 * @param {Function} callback Function to call once the operation completes
 */
function onAlreadyRegisteredDataSearch(data, callback, err, result) {
  if (err && callback) {
    return process.nextTick(callback.bind(null, err));
  }
  if (result) {
...
function getNotificationInfo(data, callback) {
  ...
  collection.findOne(
    findCondition,
    onAlreadyRegisteredDataSearch.bind(null, data, callback)
  );
}
