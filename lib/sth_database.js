 ...
 *  - {string} entityType The type of entity related to the event
 *  - {string} attrName The attribute id related to the event
 *  - {string} aggregationType The aggregation type
 * @returns {string} The collection name
 */
function getCollectionName4Aggregated(params) {
  ...
  if (collectionName4Events) {
    return collectionName4Events + '.' + params.aggregationType + '_aggr';
  } else {
    return null;
  }
}
...
function storeAggregatedData(data, callback) {
  ...
  function onCompletion(err) {
    error = err;
    if (++counter === sthConfig.AGGREGATE_BY.length) {
      callback(err);
    }
  }
  ...
  sthConfig.AGGREGATE_BY.forEach(function (entry) {
