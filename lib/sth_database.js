...
function setTTLPolicy(collection) {
  ...
  if (sthConfig.TRUNCATION_EXPIREAFTERSECONDS > 0) {
    if (!isAggregated(collection)) {
      if (sthConfig.TRUNCATION_SIZE === 0) {
        collection.ensureIndex(
          ...
          {
            expireAfterSeconds: sthConfig.TRUNCATION_EXPIREAFTERSECONDS
          }, function (err) {
            if (err) {
              sthLogger.error(
                sthConfig.LOGGING_CONTEXT.DB_LOG,
...
function getAggregatePrepopulatedData(attrType, attrValue, resolution) {
  ...
  if (sthHelper.getAggregationType(attrValue) === sthConfig.AGGREGATIONS.NUMERIC) {
...
function getAggregateUpdate4Removal(data) {
  ...
  if (sthHelper.getAggregationType(notificationInfo.updates.attrValue) === sthConfig.AGGREGATIONS.NUMERIC) {
...
function generateNotificationInfo(data, searchResult, callback) {
  if (searchResult) {
    if (_.isEqual(searchResult.attrValue, data.attribute.value)) {
      ...
      if (sthHelper.getAggregationType(searchResult.attrValue) === sthConfig.AGGREGATIONS.NUMERIC) {
