...
function storeAggregatedData4Resolution(data, callback) {
  ...
  collection.update(
    ...
    function onAggregatedDataPrepopulated(err) {
      if (err && callback) {
      ...
      collection.update(
        ...
        function onAggregatedDataStored(err) {
          // Undo the previously aggregated data if any
          if (notificationInfo.updates) {
            collection.update(
              getAggregateUpdateCondition(
                {
                  entityId: entityId,
              ...
              function onAggregatedDataRemoved(err) {
                if (callback) {
...
function getNotificationInfo(data, callback) {
  ...
  collection.findOne(
    ...
    function(err, result) {
      ...
      if (result) {
        if (result.attrValue !== attribute.value) {
