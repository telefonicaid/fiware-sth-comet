...
var sthServer = require('./sth_server');
...
function exitGracefully(err, callback) {
  function onStopped() {
    ...
    if (err) {
      ...
      sthLogger.info(
        {
          op: sthConfig.OPERATION_TYPE.SHUTDOWN
        },
...
function startup(callback) {
  ...
  sthLogger.info(
    {
      op: sthConfig.OPERATION_TYPE.SERVER_START
    },
  ...
  sthLogger.info(
    ...
    JSON.stringify(sthConfig, null, 2)
  );
  ...
  sthDatabase.connect(
    ...
    function (err) {
      ...
      sthLogger.info(
        {
          op: sthConfig.OPERATION_TYPE.DB_CONN_OPEN
        },
...
if (!module.parent) {
  startup(function () {
    proofOfLifeInterval = setInterval(function () {
      sthLogger.info(
        {
          op: sthConfig.OPERATION_TYPE.SERVER_LOG
        },
