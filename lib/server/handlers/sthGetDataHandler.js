    ...
    sthDatabase.getCollection(query, sthOptions, function(err, collection) {
        if (err) {
            if (err.name === 'MongoConnectionError') {
                const message = 'MongoDB is not connected';
                sthLogger.error(
                    request.sth.context,
            ...
            sthDatabase.getRawData(rawQuery, function(err, result, totalCount) {
                ...
                if (err) {
                    ...
                    if (reply) {
                        response = reply.response(new stream.Readable().wrap(result));
                    } else {
                        response = replyHandler(new stream.Readable().wrap(result));
                    }
 ...
 * @param {Function} reply   Hapi's reply function
 */
function getAggregatedData(request, reply, replyHandler) {
    let response;
