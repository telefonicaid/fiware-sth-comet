...
function getCollection(params, options, callback) {
    ...
    const dbConnected = isConnectionAlive(callback
        , function(){
...
/**
 * Check whether database is connected or not, if not connected then connect to database
 * @param {function} callback The callback
 * @param {function} callbackConnection The callback
 */
function isConnectionAlive(callback, callbackConnection){
...
function fetchCollection(databaseName, isAggregated, shouldTruncate, shouldCreate, collectionName, callback){
    ...
    const connection = db.db(databaseName);
...
function dropCollection(collectionName, service, callback) {
    //Drop collection after creating database connection if connection was not restored after database restarted
    const dbConnected = isConnectionAlive(callback
        , function(){
...
/**
 * Call deleteDataDroppingCollections after creating database connection if connection was not restored after database restarted
 * @param {object} data It is an object including the following properties:
 *  - {boolean} aggregated Flag indicating if the collection to drop refers to a aggregated data one or
 *      not (to a raw data one)
 *  - {string} service The service associated to the collection to drop
...
function removeDataDroppingCollections(data, options, callback) {
    const dbConnected = isConnectionAlive(callback
        , function(){
            deleteDataDroppingCollections(data, options, callback);
        });
        if(dbConnected){
            deleteDataDroppingCollections(data, options, callback);
        }
 ...
 * @param callback The callback to call with possible errors and the result of the operation
 */
function deleteDataDroppingCollections(data, options, callback) { 
    const service = data.service;
