    ...
    if (err) {
      ...
      (collection.s.name !==
        (sthConfig.NAME_ENCODING ?
          ...
          sthConfig.COLLECTION_PREFIX + 'collection_names')) && !isAggregated) {
      setRawDataUniqueIndex(collection);
      if (shouldTruncate) {
        setTTLPolicy(collection);
      }
...
storeCollectionHash = function storeCollectionHash(params, isAggregated, hash, callback) {
  getCollection(
    {
      service: params.service,
      collection: sthConfig.NAME_ENCODING ?
                    sthDatabaseNameCodec.encodeCollectionName(sthConfig.COLLECTION_PREFIX + 'collection_names') :
...
function dropHashedCollectionIfComplies(hashedCollectionData, data, options, callback) {
  ...
  if (hashMappingResult && hashMappingResult.isAggregated === isAggregated &&
    ...
    db.db(sthDatabaseNaming.getDatabaseName(service)).dropCollection(
      collectionName,
      function(err) {
        if (err) {
          return process.nextTick(callback.bind(null, err));
