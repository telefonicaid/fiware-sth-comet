...
var STHWritableStream = require('./sth_database_writable_stream');
var sthConfig = require('./sth_configuration');
var sthError = require('./sth_error');
 ...
 * @param  {Function} callback The callback
 */
function databaseFilter(databaseData, callback) {
  process.nextTick(callback.bind(null, null, databaseData.name.indexOf(sthConfig.DB_PREFIX) !== -1));
}
 ...
 * @param  {Function} callback  The callback
 */
function filterDatabases(databases, callback) {
  async.filter(databases, databaseFilter, callback);
}
...
function isCollectionPerServicePath(collection, callback) {
  if (collection.s.name === sthConfig.DB_PREFIX + 'collection_names') {
 ...
 * @param  {Function} callback   The callback
 */
function collectionFilter(collection, callback) {
  if (collection.s.name === sthConfig.DB_PREFIX + 'collection_names') {
 ...
 * @param  {Object}   collections The collection
 * @param  {Function} callback    The callback
 */
function filterCollections(collections, callback) {
  async.reject(collections, collectionFilter, callback);
}
 ...
 * @param  {Object}   database The data about the database
 * @param  {Function} callback The callback
 */
function analyseDatabase(database, callback) {
 ...
 * @param  {Function} callback  The callback
 */
function analyseDatabases(databases, callback) {
  async.map(databases, analyseDatabase, callback);
}
 ...
 * @param  {Function} callback The callback
 */
function getDataModelAnalysis(callback) {
  async.waterfall(
    [
...
function collectionExists(databaseName, collectionName, callback) {
  ...
  db.listCollections({name: collectionName}).toArray(function(err, items) {
    process.nextTick(callback.bind(null, err, items && !!items.length));
  });
}
...
function getTargetCollectionName(originCollectionParams) {
  if (originCollectionParams.isAggregated) {
    return sthDatabase.getCollectionName4Aggregated(originCollectionParams);
  } else {
    return sthDatabase.getCollectionName4Events(originCollectionParams);
  }
}

/**
 * Returns asynchronously the target collection data in case of collection name hashing for a collection-per-entity to
 *  collection-per-service-path migration
 * @param  {String}   databaseName         The database name
 * @param  {String}   originCollectionName The origin collection name
 * @param  {Object}   hashCollection       The hash to collection data collection
 * @param  {Function} callback             The callback
 */
function getTargetCollectionDataIfHashFromCpE2CpSP(databaseName, originCollectionName, hashCollection, callback) {
  var originCollectionNameParams;
  hashCollection.findOne(
    { _id: originCollectionName },
    function(err, result) {
      if (err) {
 ...
 * @param  {Function} callback             The callback
 */
function getTargetCollectionDataFromCpE2CpSP(databaseName, originCollectionName, callback) {
  if (sthConfig.SHOULD_HASH) {
    var db = sthDatabase.connection.db(databaseName);
    async.waterfall(
      [
        async.apply(db.collection.bind(db), sthConfig.COLLECTION_PREFIX + 'collection_names'),
        async.apply(getTargetCollectionDataIfHashFromCpE2CpSP, databaseName, originCollectionName)
      ],
      callback
    );
  } else {
    var isAggregated = sthDatabase.isAggregated(originCollectionName),
        cleanedOriginCollectionName = originCollectionName.substring(sthConfig.COLLECTION_PREFIX.length,
          originCollectionName.length - (isAggregated ? '.aggr'.length : 0)),
        cleanedOriginCollectionNameSplit = cleanedOriginCollectionName.split('_'),
        servicePath,
        originCollectionNameParams;
    switch (cleanedOriginCollectionNameSplit.length) {
      ...
      case 2:
        ...
        originCollectionNameParams = {
          databaseName: databaseName,
          service: databaseName.slice(sthConfig.DB_PREFIX.length),
...
/**
 * Returns the distinct entity ids present in a collection
 * @param  {Object}   collection   The collection
 * @param  {Function} callback     The callback
 */
function getDistinctEntityIds(collection, callback) {
  var isAggregated = sthDatabase.isAggregated(collection.s.name);

  collection.distinct(isAggregated ? '_id.entityId' : 'entityId',
    function onDistictEntityIds(err, distinctEntityIds) {
      process.nextTick(callback.bind(null, err, collection, distinctEntityIds));
    }
  );
}

/**
 * Maps entity ids to their correspondent entity type for a given collection
 * @param  {Object}   collection The collection
 * @param  {String}   entityId   The entity id
 * @param  {Function} callback   The callback
 */
function entityDataMapper(targetCollectionData, collection, entityId, callback) {
  var entityTypeQuery = {},
      entityType,
      collectionNameParams = {
        databaseName: targetCollectionData.databaseName,
        servicePath: targetCollectionData.servicePath,
        entityId: entityId
      };

  if (targetCollectionData.isAggregated) {
    entityTypeQuery['_id.entityId'] = entityId;
  } else {
    entityTypeQuery.entityId = entityId;
  }
  collection.findOne(entityTypeQuery, function(err, result) {
    if (err) {
      return process.nextTick(callback.bind(null, err));
    }
    entityType = targetCollectionData.isAggregated ? result._id.entityType : result.entityType;
    collectionNameParams.entityType = entityType;
    return process.nextTick(callback.bind(null, null,
      {
        entityId: entityId,
        entityType: entityType,
        collectionName: targetCollectionData.isAggregated ?
...
/**
 * Asynchronously returns the entity data associated to a collection
 * @param  {Object}   targetCollectionData The object where the entity data should be added
 * @param  {Object}   collection           The collection
 * @param  {Array}    entityIds            The array of entity ids
 * @param  {Function} callback             The callback
 */
function addEntityData(targetCollectionData, collection, entityIds, callback) {
  async.map(entityIds, entityDataMapper.bind(null, targetCollectionData, collection),
    function onEntityData(err, entityData) {
      if (err) {
        return process.nextTick(callback.bind(null, err));
      }
      targetCollectionData.entityData = entityData;
      process.nextTick(callback.bind(null, null, targetCollectionData));
    }
  );
}

function getServicePathIfHash(collectionName, callback) {
  sthDatabase.connection.collection(sthConfig.COLLECTION_PREFIX + 'collection_names', function(err, hashCollection) {
    if (err) {
      return process.nextTick(callback.bind(null, err));
    }
    hashCollection.findOne(
      ...
      function(err, collectionData) {
        ...
        process.nextTick(callback.bind(null, null, collectionData.servicePath));
      }
    );
  });
}

function addServicePath4CpSP(collectionName, targetCollectionData, callback) {
  var isAggregated = sthDatabase.isAggregated(collectionName);
  if (sthConfig.SHOULD_HASH) {
    getServicePathIfHash(collectionName, function(err, servicePath) {
      if (err) {
        return process.nextTick(callback.bind(null, err));
      }
      targetCollectionData.servicePath = servicePath;
      return process.nextTick(callback.bind(null, null, targetCollectionData));
    });
  } else {
    targetCollectionData.servicePath = collectionName.substring(sthConfig.COLLECTION_PREFIX.length,
      collectionName.length - (isAggregated ? '.aggr'.length : 0));
    process.nextTick(callback.bind(null, null, targetCollectionData));
  }
}

/**
 * Returns asynchronously the target collection data for a collection-per-service-path to collection-per-entity
 *  migration
 * @param  {String}   databaseName         The database name
 * @param  {String}   originCollectionName The origin collection name
 * @param  {Function} callback             The callback
 */
function getTargetCollectionDataFromCpSP2CpE(databaseName, originCollectionName, callback) {
  var isAggregated = sthDatabase.isAggregated(originCollectionName),
      db = sthDatabase.connection.db(databaseName),
  ...
  async.waterfall(
    [
      async.apply(addServicePath4CpSP, originCollectionName, targetCollectionData),
      async.apply(db.collection.bind(db), originCollectionName),
      getDistinctEntityIds,
      async.apply(addEntityData, targetCollectionData)
    ],
    callback
  );
}
...
/**
 * Removes an entry from the hash collection
 * @param  {String}   collectionName The name of the collection to remove
 * @param  {Object}   hashCollection The hash collection
 * @param  {Function} callback       The callback
 */
function removeEntryFromHashCollection(collectionName, hashCollection, callback) {
...
function doMigrateFromCpE2CpSP(params, options, targetCollectionData, callback) {
  ...
  if (sthConfig.SHOULD_HASH) {
    functions.push(
      async.apply(db.collection.bind(db), sthConfig.COLLECTION_PREFIX + 'collection_names'),
      async.apply(removeEntryFromHashCollection, targetCollectionData.collectionName),
      async.apply(sthDatabase.storeCollectionHash, targetCollectionData, targetCollectionData.isAggregated,
        targetCollectionData.collectionName)
    );
  }
  functions.push(
...
function transformFromCpSP2CpE(isAggregated, doc) {
  if (isAggregated) {
    delete doc._id.entityId;
    delete doc._id.entityType;
  } else {
    delete doc.entityId;
 ...
 * @param  {Function} callback    The callback
 */
function doPipeFromCpSP2CpE(params, options, entityData, originCount, callback) {
  var sthWritableStream = new STHWritableStream(
    sthDatabase.connection, params.databaseName, entityData.collectionName, originCount);
  var originQuery = {};
  if (params.targetCollectionData.isAggregated) {
    originQuery['_id.entityId'] = entityData.entityId;
    originQuery['_id.entityType'] = entityData.entityType;
  } else {
    originQuery.entityId = entityData.entityId;
    originQuery.entityType = entityData.entityType;
  }
  params.originCollection.find(originQuery).stream(
    {
      transform: transformFromCpSP2CpE.bind(null, params.targetCollectionData.isAggregated)
    }
  ).
  pipe(sthWritableStream).
  // on('progress', params.collectionMigrationProgress.emit.bind(params.collectionMigrationProgress, 'progress')).
  on('progress', function(progress) {
    params.collectionMigrationProgress.overallCounter = params.collectionMigrationProgress.overallCounter || 0;
    params.collectionMigrationProgress.overallCounter++;
    params.collectionMigrationProgress.emit(
      'progress', { total: originCount, count: params.collectionMigrationProgress.overallCounter })
  }).
  on('finish', callback);
}

/**
 * Pipes a the entity data associated to a collection-per-service-path to the collection-per-entity data model
 * @param  {Object}   params           Object enclosing the following params taken by this function as input:
 *                                       - collectionMigrationProgress EventEmitter to notify progress events
 *                                       - databaseName The database name
 *                                       - originCollectionName: The origin collection name
 ...
 * @param  {Function} callback         The callback
 */
function pipeFromCpSP2CpE(params, options, entityData, originCollection, callback) {
  params.originCollection = originCollection;
  async.waterfall(
    [
      originCollection.count.bind(originCollection),
      async.apply(doPipeFromCpSP2CpE, params, options, entityData)
    ],
    callback
  );
}
...
/**
 * Migrates the entries associated to certain entity data from the collection-per-service-path to the
 * collection-per-entity data model
 * @param  {Object}   params     Object enclosing the following params taken by this function as input:
 *                                 - collectionMigrationProgress: EventEmitter to notify progress events
 *                                 - databaseName: The database name
 *                                 - originCollectionName: The origin collection name
 *                                 - targetCollectionData: The data associated to the target collections
 * @param  {Object}   options    The migration options object including the following properties:
 *                                 - removeCollection: Boolean indicating if the origin collection should be
 *                                                       removed after the migration
 *                                 - updateCollection: Boolean indicating if the migration progress should
 ...
 * @param  {Function} callback   The callback
 */
function migrateEntityData(params, options, entityData, callback) {
  var db = sthDatabase.connection.db(params.databaseName);
  var functions = [];
  if (sthConfig.SHOULD_HASH) {
    functions.push(
      async.apply(db.collection.bind(db), sthConfig.COLLECTION_PREFIX + 'collection_names'),
      async.apply(removeEntryFromHashCollection, entityData.collectionName),
      async.apply(
        sthDatabase.storeCollectionHash,
        {
          service: params.targetCollectionData.service,
          servicePath: params.targetCollectionData.servicePath,
          entityId: entityData.entityId,
          entityType: entityData.entityType
        },
        params.targetCollectionData.isAggregated,
  ...
  functions.push(
    async.apply(db.collection.bind(db), params.originCollectionName),
    async.apply(pipeFromCpSP2CpE, params, options, entityData)
  );
  async.waterfall(functions, callback);
}

/**
 * Migrates a collection from the collection-per-service-path data model to the collection-per-entity data model
 * @param  {Object}   params               Object enclosing the following params taken by this function as input:
 *                                           - collectionMigrationProgress: EventEmitter to notify progress events
 *                                           - databaseName: The database name
 *                                           - originCollectionName: The origin collection name
 ...
 *                                                                 take place even if the target collection already
 *                                                                 exists
 * @param  {Object}   targetCollectionData The data about the target collections
 * @param  {Function} callback             The callback
 */
function doMigrateFromCpSP2CpE(params, options, targetCollectionData, callback) {
  params.targetCollectionData = targetCollectionData;
  async.each(
    targetCollectionData.entityData,
    migrateEntityData.bind(null, params, options),
    callback
  );
}
...
function migrateFromCpE2CpSP(params, options, callback) {
  async.waterfall(
    [
      async.apply(getTargetCollectionDataFromCpE2CpSP, params.databaseName, params.originCollectionName),
...
function existsTargetCollectionFromCpSP2CpE(targetCollectionData, entityDataItem, callback) {
  collectionExists(targetCollectionData.databaseName, entityDataItem.collectionName, function(err, collectionExists) {
    if (err) {
      return process.nextTick(callback.bind(null, err));
    }
    return process.nextTick(callback.bind(null, null, collectionExists));
  });
}

function checkTargetCollectionDataFromCpSP2CpE(
  originCollectionName, options, targetCollectionData, callback) {
  if (!options.updateCollection) {
    async.detect(
      targetCollectionData.entityData,
      existsTargetCollectionFromCpSP2CpE.bind(null, targetCollectionData),
      function(err, entityDataItem) {
        if (err) {
          return process.nextTick(callback.bind(null, err));
        }
        if (entityDataItem) {
          process.nextTick(callback.bind(null,
            new sthError.CollectionExistsError(targetCollectionData.databaseName, entityDataItem.collectionName)));
        } else {
          process.nextTick(callback.bind(null, null, targetCollectionData));
        }
      }
 ...
 * @param  {Function} callback The callback
 */
function migrateFromCpSP2CpE(params, options, callback) {
  async.waterfall(
    [
      async.apply(getTargetCollectionDataFromCpSP2CpE, params.databaseName, params.originCollectionName),
      async.apply(checkTargetCollectionDataFromCpSP2CpE, params.originCollectionName, options),
      async.apply(doMigrateFromCpSP2CpE, params, options)
    ],
    callback
  );
}
...
function migrate2DataModel(params, options, originDataModel, callback) {
  switch(originDataModel) {
    ...
    case sthConfig.DATA_MODELS.COLLECTION_PER_SERVICE_PATH:
      switch (sthConfig.DATA_MODEL) {
        case sthConfig.DATA_MODELS.COLLECTION_PER_ENTITY:
          migrateFromCpSP2CpE(params, options, callback);
          break;
        default:
        return process.nextTick(callback.bind(null, new sthError.NotSupportedMigration(
          params.databaseName, params.originCollectionName, originDataModel, sthConfig.DATA_MODEL)));
