...
/**
 * Maps entity ids to their correspondent entity type
 * @param  {Object}   collection The collection
 * @param  {String}   entityId   The entity id
 * @param  {Function} callback   The callback
 */
function entityIdTypeMapper(collection, entityId, callback) {
  var isAggregated = sthDatabase.isAggregated(collection.s.name);
  ...
  collection.findOne(entityTypeQuery, function(err, result) {
    if (err) {
      return process.callback(null, err);
    }
    return process.nextTick(callback.bind(null, null,
      {
        entityId: entityId,
        entityType: isAggregated ? result._id.entityType : result.entityType
      }
    ));
  });
}
...
function addEntityData(targetCollectionData, collection, entityIds, callback) {
  async.map(entityIds, entityIdTypeMapper.bind(null, collection), function onEntityIdTypeArray(err, entityIdTypeArray) {
    if (err) {
      return process.nextTick(callback.bind(null, err));
    }
    targetCollectionData.entityData = entityIdTypeArray;
...
function getTargetCollectionDataFromCpSP2CpE(databaseName, originCollectionName, callback) {
  ...
  async.waterfall(
    [
      ...
      async.apply(addServicePath4CpSP, originCollectionName)
    ],
    callback
  );
}
...
function migrateFromCpSP2CpE(params, options, callback) {
  async.waterfall(
    [
      ...
      function(targetCollectionData, callback) {
        console.log(targetCollectionData);
      }
      // async.apply(targetCollectionDataCheck, params.databaseName, options),
      // async.apply(doMigrateFromCpE2CpSP, params, options)
    ],
