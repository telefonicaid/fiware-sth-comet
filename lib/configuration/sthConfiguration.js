...
let invalidLoggingLevel;
const LOGGING_LEVELS = ['DEBUG', 'INFO', 'WARN', 'ERROR', 'FATAL'];
...
let invalidLoggingFormat;
const LOGGING_FORMATS = ['json', 'dev', 'pipe'];
...
if (ENV.CORS_ENABLED && !(ENV.CORS_ENABLED == null)) {
    ...
    module.exports.corsEnabled = false;
    sthLogger.info(
        module.exports.LOGGING_CONTEXT.STARTUP,
        'Not configured CORS, setting to default CORS value: ' + module.exports.corsEnabled
    );
}
module.exports.corsOptions = config.cors.options;
...
let isEnvAggregationBy = false;
let envAggregationByParseError = false;
if (ENV.AGGREGATION_BY) {
    try {
        const envAggregationBy = JSON.parse(ENV.AGGREGATION_BY);
...
module.exports.DB_AUTH_SOURCE = ENV.DB_AUTH_SOURCE || '';
if (ENV.DB_AUTH_SOURCE) {
    sthLogger.info(
        module.exports.LOGGING_CONTEXT.STARTUP,
        'Database auth source set to value: ' + module.exports.DB_AUTH_SOURCE
    );
}
...
if (ENV.DB_RECONNECT_TRIES && !isNaN(ENV.DB_RECONNECT_TRIES) && parseInt(ENV.DB_RECONNECT_TRIES, 10) > 0) {
    module.exports.DB_RECONNECT_TRIES = parseInt(ENV.DB_RECONNECT_TRIES, 10);
    sthLogger.info(
        module.exports.LOGGING_CONTEXT.STARTUP,
        'Database reconnect tries set to value: ' + module.exports.DB_RECONNECT_TRIES
    );
} else if (
    config.database.reconnectTries &&
    !isNaN(config.database.reconnectTries) &&
    parseInt(config.database.reconnectTries, 10) > 0
) {
    module.exports.DB_RECONNECT_TRIES = parseInt(config.database.reconnectTries, 10);
    sthLogger.info(
        module.exports.LOGGING_CONTEXT.STARTUP,
        'Database reconnect tries set to value: ' + module.exports.DB_RECONNECT_TRIES
    );
} else {
    module.exports.DB_RECONNECT_TRIES = 30;
    sthLogger.warn(
        module.exports.LOGGING_CONTEXT.STARTUP,
        'Invalid or not configured database reconnect tries, setting to default value: ' +
            module.exports.DB_RECONNECT_TRIES
    );
}

if (ENV.DB_RECONNECT_INTERVAL && !isNaN(ENV.DB_RECONNECT_INTERVAL) && parseInt(ENV.DB_RECONNECT_INTERVAL, 10) > 0) {
    module.exports.DB_RECONNECT_INTERVAL = parseInt(ENV.DB_RECONNECT_INTERVAL, 10);
    sthLogger.info(
        module.exports.LOGGING_CONTEXT.STARTUP,
        'Database reconnect interval set to value: ' + module.exports.DB_RECONNECT_INTERVAL
    );
} else if (
    config.database.reconnectInterval &&
    !isNaN(config.database.reconnectInterval) &&
    parseInt(config.database.reconnectInterval, 10) > 0
) {
    module.exports.DB_RECONNECT_INTERVAL = parseInt(config.database.reconnectInterval, 10);
    sthLogger.info(
        module.exports.LOGGING_CONTEXT.STARTUP,
        'Database reconnect interval set to value: ' + module.exports.DB_RECONNECT_INTERVAL
    );
} else {
    module.exports.DB_RECONNECT_INTERVAL = 1000;
    sthLogger.warn(
        module.exports.LOGGING_CONTEXT.STARTUP,
        'Invalid or not configured database reconnect interval, setting to default value: ' +
            module.exports.DB_RECONNECT_INTERVAL
    );
}
